  ...
  <v-container>
    <v-form ref="form" v-model="valid" lazy-validation>
      <v-row>
        <v-col lg="12">
          <v-text-field
            :rules="[(v) => !!v || 'Judul Proposal harus diisi']"
            v-model="build.title"
            @keyup="build.title = capitalizeFirstLetter(build.title)"
            dense
      ...
      <v-row>
        <v-col lg="6">
          <v-select
            ...
            v-model="build.educational_level"
            item-value="id"
            item-text="name"
            label="Pilih Jenjang"
          ></v-select>
...
export default {
  ...
  created() {
    ...
    const find = this.ClassroomResearch.contents.find(
      (e) => e.educational_level_id == this.build.educational_level
    );
    if (find) {
  ...
  methods: {
    ...
    getTemplate() {
      axios
        .get(
          "/educational_level/" +
            this.build.educational_level +
            "/classroom_research_format"
        )
        .then((res) => {
          const find = this.ClassroomResearch.contents.find(
            (e) => e.educational_level_id == res.data.id
          );
